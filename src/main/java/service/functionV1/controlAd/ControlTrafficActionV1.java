...
/**
 * V1产品层面控制流量处理类
 * 负责读取控制流量Excel文件，按站点处理MSKU，降低Bid
 * 
 * @author <a href="mailto:qinglong.xql@cainiao.com">qinglong.xql</a>
 * @version 1.0
 * @since 2025/01/XX
 */
public class ControlTrafficActionV1 {
    ...
    public ProcessResult execute(Configuration configuration) {
        ...
        System.out.println("========== 开始处理V1产品层面控制流量 ==========");
        
        // 读取控制流量Excel文件
        Map<String, List<String>> mskuMap = MskuExcelReader.readMskuExcel(FilePath.v1SkuControlExcel);
        
        if (mskuMap.isEmpty()) {
            System.out.println("控制流量Excel文件为空或不存在，跳过处理");
            return result;
        }
        ...
        for (Map.Entry<String, List<String>> entry : mskuMap.entrySet()) {
            ...
            for (int i = 0; i < mskuList.size(); i++) {
                ...
                for (AdGroup adGroup : adGroupList) {
                    ...
                    // 判断近3天点击是否>0
                    AdGroupRequest recentRequest = new AdGroupRequest();
                    recentRequest.setProfile_id(Long.parseLong(adGroup.getProfile_id()));
                    recentRequest.setCampaign_id(Long.parseLong(adGroup.getCampaign_id()));
                    recentRequest.setReport_date(DateUtils.buildReportDateString(2)); // 近3天
                    AdGroup recentAdGroup = new AdGroupReadRepository().queryAdGroupListInAdGroupPage(recentRequest, configuration);
