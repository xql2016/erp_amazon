...
public class CategoryAdGroupDetailAction {
    ...
    public void executeAdGroupDetail(AdGroup newAdGroup, AdGroupType adGroupType, Configuration configuration, int realDays) {
        String key1 = "V2类目广告组层面长期ACOS控制基准下限";
        int value1 = configuration.getFeatures().getIntValue(key1);
        String key2 = "V2类目广告组层面长期ACOS控制基准上限";
        ...
        if((null == newAdGroup.getOrders() || 0 == newAdGroup.getOrders()) && newAdGroup.getClicks() < 10) {
            // 不处理,打日志
            System.out.println(String.format("    当前广告组广告订单数为0且点击数小于10,无需变更, country=%s, adGroupName=%s, acos =%s", newAdGroup.getStore_country(), newAdGroup.getName(), newAdGroup.getAcos()));
            return;
        }
    ...
    private void handle1(AdGroup newAdGroup, AdGroupType adGroupType, AdPlacement adPlacement, Configuration configuration) {
        if (adPlacement.getClicks() >= 20) {
            ...
            // 不处理,打日志
            System.out.println(String.format("   广告组无广告订单且点击数处于10-30且投放入口点击数小于4,无需变更, country=%s, adGroupName=%s, placementName=%s， acos =%s, click=%s, cpc=%s", newAdGroup.getStore_country(), adPlacement.getAd_group_name(), new AdPlacementUtils().getAdPlacementName(adGroupType, adPlacement), adPlacement.getAcos(), adPlacement.getClicks(), adPlacement.getCpc()));
        }
    }
    ...
    private void handle3(AdGroup newAdGroup, AdGroupType adGroupType, AdPlacement adPlacement, Configuration configuration, int value1, int value2) {
        if (adPlacement.getOrders() > 0) {
            ...
            if (adPlacement.getClicks() >= 20) {
                ...
                // 不处理,打日志
                System.out.println(String.format("   广告组有广告订单且acos处于%s-60且投放入口无订单且投放入口点击数小于4,无需变更, country=%s, adGroupName=%s, placementName=%s， acos =%s, click=%s, cpc=%s", value1, newAdGroup.getStore_country(), adPlacement.getAd_group_name(), new AdPlacementUtils().getAdPlacementName(adGroupType, adPlacement), adPlacement.getAcos(), adPlacement.getClicks(), adPlacement.getCpc()));
            }
        }
    }
