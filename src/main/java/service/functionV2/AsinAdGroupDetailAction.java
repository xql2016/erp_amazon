...
public class AsinAdGroupDetailAction {
    ...
    public void executeAdGroupDetail(AdGroup newAdGroup, AdGroupType adGroupType, Configuration configuration, int realDays) {
        String key1 = "V2ASIN广告组层面长期ACOS控制基准下限";
        int value1 = configuration.getFeatures().getIntValue(key1);
        String key2 = "V2ASIN广告组层面长期ACOS控制基准上限";
        ...
        for(AdPlacement adPlacement : adPlacementList) {
            ...
            if(null == newAdGroup.getOrders() || 0 == newAdGroup.getOrders()) {
                // 无广告订单处理
                if (newAdGroup.getClicks() >= 7 && newAdGroup.getClicks() < 30) {
    ...
    private void handle1(AdGroup newAdGroup, AdGroupType adGroupType, AdPlacement adPlacement, Configuration configuration) {
        if (adPlacement.getClicks() >= 10) {
            new AdPlacementUtils().close(newAdGroup, adGroupType, adPlacement, configuration);
        } else if (adPlacement.getClicks() >= 4) {
            new AdPlacementUtils().subtractBid(newAdGroup, adGroupType, adPlacement, Double.parseDouble(adPlacement.getCpc()) + 0.04 - 0.01 * adPlacement.getClicks(), configuration);
    ...
    private void handle2(AdGroup newAdGroup, AdGroupType adGroupType, AdPlacement adPlacement, Configuration configuration) {
        if(adPlacement.getClicks() >= 10) {
            ...
            new AdPlacementUtils().subtractBid(newAdGroup, adGroupType, adPlacement, Double.parseDouble(adPlacement.getCpc()) + 0.02 - 0.01 * adPlacement.getClicks(), configuration);
        } else if(adPlacement.getClicks() >= 1) {
