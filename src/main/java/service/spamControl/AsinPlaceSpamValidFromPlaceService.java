...
public class AsinPlaceSpamValidFromPlaceService {
    ...
    private static void doOpen(Configuration configuration, GoodsPlace goodsPlace, double bidNow) {
        ...
        if(!configuration.isDoSimulation()) {
            ...
            System.out.println(String.format("asin投放操作,打开,真实操作结果=%s, 名称=%s,bid=%s,cpc=%s,acos=%s,点击数=%s,订单数=%s,", operateResult,goodsPlace.getAd_group_name(),bidNow,goodsPlace.getCpc(),goodsPlace.getAcos(),goodsPlace.getClicks(),goodsPlace.getOrders()));
        } else {
            System.out.println(String.format("asin投放操作,打开,仿真, 名称=%s,bid=%s,cpc=%s,acos=%s,点击数=%s,订单数=%s,", goodsPlace.getAd_group_name(),bidNow,goodsPlace.getCpc(),goodsPlace.getAcos(),goodsPlace.getClicks(),goodsPlace.getOrders()));
        }
    }
