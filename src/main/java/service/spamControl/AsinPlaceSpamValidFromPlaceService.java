 ...
 * @since 2024/12/21
 */
public class AsinPlaceSpamValidFromPlaceService {

    public static GoodsPlaceRequest buildGoodsPlaceRequest(SpamValidFromPlaceSearchCondition spamValidFromPlaceSearchCondition, Long hubId, List<HubPortfolioId> hubPortfolioIdList, String expressionType) {
    ...
    // 找到符合条件的策略然后进行操作
    public static void doOperation(GoodsPlace goodsPlace, SpamValidFromPlaceDoOperation spamValidFromPlaceDoOperation, Configuration configuration, String expressionType) {
        ...
        switch (SpamValidOperateType.getByValue(action.getSpamValidOperateType())) {
            case NO_OPERATE:
                ...
                break;
            case OPEN:
                doOpen(configuration, goodsPlace, bidNow, expressionType);
                break;
    ...
    private static void doOpen(Configuration configuration, GoodsPlace goodsPlace, double bidNow, String expressionType) {
        String operation = "asinSameAs".equalsIgnoreCase(expressionType) ? "商品投放" : "商品扩展";
        ...
        if(!configuration.isDoSimulation()) {
            ...
            boolean operateResult = adWriteRepository.doAdPlacementOperateOpen(adPlacementPauseRequest, configuration, AdGroupType.ASIN_AD_GROUP);
            System.out.println(String.format("%s操作,打开,真实操作结果=%s, 名称=%s,bid=%s,cpc=%s,acos=%s,点击数=%s,订单数=%s,", operation, operateResult,goodsPlace.getAd_group_name(),bidNow,goodsPlace.getCpc(),goodsPlace.getAcos(),goodsPlace.getClicks(),goodsPlace.getOrders()));
        } else {
            System.out.println(String.format("%s放操作,打开,仿真, 名称=%s,bid=%s,cpc=%s,acos=%s,点击数=%s,订单数=%s,", operation, goodsPlace.getAd_group_name(),bidNow,goodsPlace.getCpc(),goodsPlace.getAcos(),goodsPlace.getClicks(),goodsPlace.getOrders()));
        }
    }
