...
public class CategoryAdGroupDetailAction {
    ...
    public void executeAdGroupDetail(AdGroup adGroup, AdGroupType adGroupType, Configuration configuration) {
        if(adGroup.getImpressions() > 20) {
            // 打日志,不做处理
            System.out.println(String.format("    当前广告组近3天曝光大于20,无需变更, country=%s, adGroupName=%s, 近3天曝光=%s", adGroup.getStore_country(), adGroup.getName(), adGroup.getImpressions()));
            return;
        }
        // 2. 查看广告组近60天的的数据
        ...
        if(newAdGroup.getClicks() >= 10) {
            // 打日志,不做处理
            System.out.println(String.format("    当前广告组近3天曝光小于等于20且近60天点击大于等于10,无需变更, country=%s, adGroupName=%s, 近3天曝光=%s, 近60天点击=%s", adGroup.getStore_country(), adGroup.getName(), adGroup.getImpressions(), newAdGroup.getClicks()));
            return;
        }
        Double bidValue = NumberUtils.parseDouble(adGroup.getDefault_bid());
        if(null == bidValue|| bidValue > 0.4) {
            // 打日志,不做处理
            System.out.println(String.format("    当前广告组近3天曝光小于等于20且近60天点击小于10且bid大于0.4,无需变更, country=%s, adGroupName=%s, 近3天曝光=%s, 近60天点击=%s, bid=%s", adGroup.getStore_country(), adGroup.getName(), adGroup.getImpressions(), newAdGroup.getClicks(), bidValue));
            return;
        }
        if(bidValue > 0.3) {
            // 昨天和今天有变更日志则不处理
