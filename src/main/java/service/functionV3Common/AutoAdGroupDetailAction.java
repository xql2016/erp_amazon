...
public class AutoAdGroupDetailAction {
    ...
    public void executeAdGroupDetail(AdGroup adGroup, AdGroupType adGroupType, Configuration configuration) {
        ...
        if(null == bidValue|| bidValue > 0.3) {
            // 打日志,不做处理
            System.out.println(String.format("    当前广告组近3天曝光小于等于20且近60天点击小于10且bid大于0.3,无需变更, country=%s, adGroupName=%s, 近3天曝光=%s, 近60天点击=%s, bid=%s", adGroup.getStore_country(), adGroup.getName(), adGroup.getImpressions(), newAdGroup.getClicks(), bidValue));
            return;
        }
        // 昨天和今天有变更日志则不处理
